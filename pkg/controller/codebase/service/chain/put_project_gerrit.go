...
func (h PutProjectGerrit) ServeRequest(c *v1alpha1.Codebase) error {
	...
	if err := h.tryToPushProjectToGerrit(*port, c.Name, wd, c.Namespace, c); err != nil {
		setFailedFields(c, edpv1alpha1.GerritRepositoryProvisioning, err.Error())
...
func (h PutProjectGerrit) tryToPushProjectToGerrit(sshPort int32, codebaseName, workDir, namespace string, c *v1alpha1.Codebase) error {
	...
	if err := h.git.CreateLocalBranch(d, c.Spec.DefaultBranch); err != nil {
		return errors.Wrapf(err, "creation default branch in Gerrit has been failed", c.Spec.DefaultBranch)
	}

	if err := h.git.Checkout(d, c.Spec.DefaultBranch); err != nil {
		return errors.Wrapf(err, "checkout default branch in Gerrit has been failed", c.Spec.DefaultBranch)
	}
